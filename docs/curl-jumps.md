...
**We do not yet have a measurement for this.** Our branch is not currently rebased onto upstream
main, and a retest with `rtx_bot_ledgecap 0` on top of upstream is in progress; no result exists at
the time of writing. Nothing above should be read as a claim that the cap does or does not break
the RA route in practice — only that the two mechanisms are in tension by construction, and that
