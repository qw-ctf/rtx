...
use crate::bot::state::{
    BotState, ControlOrder, GoalCommit, HookState, ItemTrial, ItemTrialMoveFrame, ItemTrialSample,
    RjOutcome, RjState, RjTelemetry,
};
...
const RA_TRIAL_LOCAL_DEFAULT_SECS: f32 = 2.435_059;
...
const RA_TRIAL_SPAWN_DEFAULT_SECS: f32 = 12.6255;
const RA_TRIAL_RING_DEFAULT_SECS: f32 = 9.604_003;
...
fn any_item_trial_active(game: &GameState) -> bool {
...
fn ensure_item_trial_idle(game: &GameState, e: EntId) -> Result<(), String> {
    item_trial_idle(game.entities[e].bot.puppet.item_trial.is_some(), e.0)
}

fn ensure_global_item_trial_idle(game: &GameState) -> Result<(), String> {
...
fn poll_ra_trial(game: &mut GameState, e: EntId, bot: u32, now: f32) {
...
mod tests {
    ...
    fn poison_trial_bot(bot: &mut BotState, n: u32) {
        ...
        bot.rj.link = n + 7;
        bot.sj = Some(Commit { leg: n + 8, since: 8.0, entry_checked: false });
